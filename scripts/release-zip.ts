...
/**
 * Build a release zip from translations
...
if (!translationSubdir) {
  log.error('Usage: vite-node scripts/release-zip.ts <translation-subdir>');
  log.error('Example: vite-node scripts/release-zip.ts example');
  process.exit(1);
}
...
log.info(`Starting release:zip workflow (translations: ${translationSubdir})`);
log.newline();
...
log.success('Release zip build complete!');
