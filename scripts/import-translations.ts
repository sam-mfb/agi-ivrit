...
function importTranslations(projectDir: string, translationSubdir?: string): void {
  ...
  if (pictureStats.copied > 0) {
    log.success(`Copied ${pictureStats.copied} picture resource(s)`);
  } else {
    log.info('No picture resources to copy');
  }

  // Copy view resources
  log.info('Copying view resources...');
  const viewStats = copyViewResources(translationsDir, finalSrcDir);
  if (viewStats.copied > 0) {
    log.success(`Copied ${viewStats.copied} view resource(s)`);
  } else {
    log.info('No view resources to copy');
  }
...
function copyPictureResources(
  ...
  // Find all .agipic files
  const picFiles = readdirSync(translationsPicDir)
    .filter(f => f.endsWith('.agipic'));

  // Copy each file
  for (const file of picFiles) {
    const srcPath = join(translationsPicDir, file);
    const destPath = join(finalPicDir, file);
    cpSync(srcPath, destPath);
  }

  return { copied: picFiles.length };
}
...
function copyViewResources(
  ...
  // Find all .agiview files
  const viewFiles = readdirSync(translationsViewDir)
    .filter(f => f.endsWith('.agiview'));

  // Copy each file
  for (const file of viewFiles) {
    const srcPath = join(translationsViewDir, file);
    const destPath = join(finalViewDir, file);
    cpSync(srcPath, destPath);
  }

  return { copied: viewFiles.length };
}
